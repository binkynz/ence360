...
struct Link* map_list(struct Link* head, int (*func)(int)) {
    struct Link* new = NULL;
    for (struct Link* item = head; item != NULL; item = item->next)
        new = append(func(item->value), new);
    return new;
}
...
void free_list(struct Link *list) {
    for (struct Link* item = list; item != NULL;) {
        struct Link* next = item->next;
        free(item);
        item = next;
    }
}
