...
float *merge_tree(int *result_size, float *sub_vec, int n1, int step)
{
    ...
    if (rank % step == 0)
    {
        ...
        else
        {
            ...
            printf("%d: received length of vector = %d from %d\n", rank, n2, source);

            other_vec = alloc_vec(n2);

            MPI_Recv(other_vec, n2, MPI_FLOAT, source, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            printf("%d: received vector = %p from %d\n", rank, other_vec, source);
    ...
    else
    {
        ...
        printf("%d: sending length of vector = %d\n", rank, n1);
        MPI_Send(&n1, 1, MPI_INT, dest, 0, MPI_COMM_WORLD);

        printf("%d: sending vector = %p\n", rank, sub_vec);
