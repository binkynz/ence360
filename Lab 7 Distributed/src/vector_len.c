...
int main(int argc, char **argv)
{
    ...
    double *results = NULL;
    if (world_rank == 0)
        results = alloc_vec(world_size);

    // send the local total to all processes and store (if root)
    MPI_Gather(&local_len, 1, MPI_DOUBLE, results, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
