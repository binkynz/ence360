...
/*
  we are maximizing cache hits by keeping data access is contiguous.
  thus a cache line is likely to contain most of the data required for a k loop.

  i.e. each k iteration is consuming from continuous memory as its j * n and not k * n
*/
...
void matrix_mul_trans_blocked(double *res, double *a, double *b, int n, int block)
{
  double *bt = alloc_matrix(n);
  matrix_transpose(bt, b, n);
