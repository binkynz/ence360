...
void benchmark_mul(int n, int repeats)
{
  ...
  for (i = 0; i < repeats; ++i)
  {
    matrix_mul_trans_blocked(res, a, b, n, 16);
  }

  clock_gettime(CLOCK_REALTIME, &stop);
  double trans_blocked = seconds(start, stop) / (double)repeats;

  printf("%d basic: %.5lf, transposed: %.5lf, blocked: %.5lf trans_blocked: %.5lf\n", n, basic, transposed, blocked, trans_blocked);
